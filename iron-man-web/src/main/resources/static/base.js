layui.use(['element','form','table','laypage','laytpl','layer','laydate','upload','carousel','util'], function () {
  ...
  var add_tab = function(elem){
    ...
    var icon_text  = $(elem).children('a').children('i').text();   //icon图标
    ...
    if (exist.length > 0) {
      ...
      element.tabAdd('main-tab', {
        title: title,
        content: '<iframe id="' + id + '" src="' + url + '" class="layui-tab-iframe" style="height: 100%;width: 100%"></iframe>',
        id: id
      });
      element.tabChange('main-tab', id);
      $('.layui-tab-item').attr('style','height: 100%')
    }
  };
  //左侧菜单点击事件
  ...
  window.updateAlert = function (text,code) {
    if(code==1){
      Dialog.Success(text);
    }else{
      Dialog.Error(text);
